...
def sample(job):
    ...
    with job:
        ...
        file_path = path.abspath(path.join(job.ws, "..", "..", job.sp.forcefield))
        if path.isfile(file_path):
            forcefield = foyer.Forcefield(forcefield_files=file_path)
            print("loading forecefield ...")
        else:
            forcefield = FORCEFIELD[job.sp.forcefield]
...
def post_proc(job):
    ...
    with gsd.hoomd.open(gsdfile, mode="rb") as f:
    	...
    	for types in all_atoms:
    		...
    		msd = plt.figure(2, figsize=(10,5.5))
